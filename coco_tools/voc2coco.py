...
def generateId(value=''):
    ...
    _value = f'{value}_{datetime.now()}'
    
    return int.from_bytes(hashlib.sha256(_value.encode('utf-8')).digest()[:4], 'little')
...
def _parseXml(annotation_path,image_path,filename,classes) :
    ...
    _img_filepath = os.path.join(image_path, root.find('filename').text)
    _img = cv2.imread(_img_filepath)
    
    imgObj["height"] = _img.shape[0]
    imgObj["width"] = _img.shape[1]
    # imgObj["id"] = uuid.uuid3(uuid.NAMESPACE_DNS, filename).int>>64
    imgObj["id"] = generateId(_img_filepath)

    metaObj = {
        'id': generateId(imgObj["id"])
    }
    ...
    for member in root.findall('object'):
        ...
        obj['id'] = generateId( str(obj['category_id']) + str(obj['bbox']) )
...
def loadVocDataset(annotation_path,image_path,classes,superset=None) :
    ...
    for idx, filename in tqdm( enumerate(os.listdir(annotation_path)) ):
        # print(filename)
...
parser.add_argument('--output-path','-o', type=str,help='help : output name')
...
#%%
os.makedirs(os.path.dirname(output_path), exist_ok=True) # make dir if not exist
...
print(f'{output_path} 저장 완료 , image num {len(_cocoData["images"])} , annotation num {len(_cocoData["annotations"])}')
